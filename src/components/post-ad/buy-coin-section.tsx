  ...
  return (
    <section className="rounded-2xl bg-white p-4 sm:p-6 w-full max-w-full overflow-hidden box-border">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 border-b border-gray-100 pb-3">
        <div>
          <h2 className="text-xl sm:text-2xl font-black text-gray-950">Buy Coins</h2>
          <p className="mt-0.5 text-xs sm:text-sm text-gray-900">
        ...
        <div className="flex items-center gap-3 text-xs shrink-0 pt-1 sm:pt-0">
          <span className="font-medium text-gray-800">
            Available: <strong className="font-black text-gray-950">{user?.coins ?? 0} coins</strong>
          </span>
          <span className="text-gray-200">•</span>
          <Link
            href="/post-ad/payment-history"
            className="font-bold text-gray-700 hover:text-gray-900 hover:underline transition"
          >
            Transactions &rarr;
          </Link>
        </div>
      ...
      {eligibility && !eligibility.allowed && (
        <div className="mt-4 rounded-xl border border-gray-300 bg-gray-50 p-4 text-gray-950 shadow-xs">
          ...
          return (
            <div
              ...
              className={`flex flex-col justify-between gap-3 rounded-2xl border p-4 sm:p-3.5 transition-all overflow-hidden w-full max-w-full box-border ${
                pkg.popular
                  ? "border-gray-300 bg-gray-50/80 shadow-xs"
            ...
            >
              <div className="min-w-0 w-full">
                ...
                <div className="flex items-center justify-between gap-1.5 mb-1.5 min-h-[22px] flex-wrap">
                  <div className="flex items-center gap-1.5 flex-wrap">
                    {pkg.popular && (
                      <span className="inline-block rounded-full bg-gray-600 px-2 py-0.5 text-[9px] font-bold uppercase tracking-wider text-white shrink-0">
                        Popular
                      </span>
                    )}
                    {pkg.label && (
                      <span className="inline-block rounded-full bg-gray-100 px-2 py-0.5 text-[9px] font-semibold text-gray-800 shrink-0">
                        {pkg.label}
                      </span>
                    )}
                  </div>
                  ...
                  {pkg.discount && pkg.discount.trim() !== "—" && pkg.discount.trim() !== "-" && (
                    <span className="inline-block rounded bg-gray-100 border border-gray-300 px-1.5 py-0.5 text-[9px] font-bold text-gray-900 shrink-0">
                      {pkg.discount}
                    </span>
                  )}
                </div>
                ...
                {/* Coins Number */}
                <p className="text-xl sm:text-lg lg:text-xl font-black text-gray-950 leading-tight">
                  {pkg.coins}
                  <span className="ml-1.5 text-xs sm:text-[11px] font-semibold text-gray-700">
                    coins
                  </span>
                </p>
                ...
                {pkg.breakdown && (
                  <p className="mt-1 text-xs sm:text-[10px] font-bold text-gray-800 break-words">
                    {pkg.breakdown}
                  </p>
                )}
                ...
                <div className="mt-1.5 leading-snug">
                  ...
                  <span className="text-sm sm:text-xs lg:text-[13px] font-bold text-gray-900 break-words">
              ...
              <Button
                ...
                className={`w-full py-1.5 text-xs font-bold mt-auto shrink-0 shadow-xs ${
                  eligibility && !eligibility.allowed
                    ? "!border-gray-400 !text-gray-900 bg-gray-50/80 hover:bg-gray-100"
                    : "!text-white"
                }`}
